(function () {
	...
	// (30:4) {#each $cart.items as item (item.key)}
	function create_each_block(key_1, ctx) {
		var li, div, input, input_value_value, t0, button, t2, figure, img, img_alt_value, img_src_value, t3, figcaption, h2, t4_value = ctx.products[ctx.item.key].name, t4, t5, b, t6, t7_value = ctx.products[ctx.item.key].price * ctx.item.count, t7, dispose;
		...
		return {
			...
			p(changed, new_ctx) {
				...
				if ((changed.$cart) && t4_value !== (t4_value = ctx.products[ctx.item.key].name)) {
					set_data(t4, t4_value);
				}

				if ((changed.$cart) && t7_value !== (t7_value = ctx.products[ctx.item.key].price * ctx.item.count)) {
