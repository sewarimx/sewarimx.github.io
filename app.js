(function () {
	...
	if (window.localStorage.cart$) {
	  skip = true;
	  cart.update(function () { return JSON.parse(window.localStorage.cart$); });
	  skip = false;
	}
	...
	function create_else_block(ctx) {
		...
		return {
			c() {
				div_1 = element("div");
				div_1.innerHTML = `<h2>No items in your basket...</h2>
			        <div><img src="viznaga_logo.png"></div>`;
				div_1.className = "wip nosl";
			},
	...
	// (31:4) {#each $cart.items as item (item.name)}
	function create_each_block(key_1, ctx) {
		var li, div, input, input_value_value, t0, button, t2, figure, img, img_alt_value, img_src_value, t3, figcaption, h2, t4_value = ctx.item.name, t4, dispose;
	...
	function create_fragment(ctx) {
		var h1, t1, div, ul, each_blocks = [], each_1_lookup = new Map(), t2, aside, h2, t4, p, t6, form, dispose;
		...
		return {
			c() {
				...
				t6 = space();
				form = element("form");
				form.innerHTML = `<label><span>Field</span>
			        <input type="text"></label>
			      <label><span>Field</span>
			        <input type="email"></label>
			      <label><span>Field</span>
			        <textarea rows="6"></textarea></label>
			      <button class="solid-shadow" type="submit">Send request</button>`;
				ul.className = "reset";
