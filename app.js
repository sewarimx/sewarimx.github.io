(function () {
	...
	function create_else_block(ctx) {
		...
		return {
			c() {
				div = element("div");
				div.innerHTML = `<h2>No items in your basket...</h2>`;
				div.className = "wip nosl";
			},
	...
	// (42:4) {#each $cart.items as item (item.key)}
	function create_each_block(key_1, ctx) {
		var li, div, button0, t1, input, input_value_value, t2, button1, t4, button2, t6, figure, img, img_alt_value, img_src_value, t7, figcaption, h2, t8_value = ctx.products[ctx.item.key].name, t8, t9, b, t10, t11_value = ctx.products[ctx.item.key].price * ctx.item.count, t11, dispose;
		...
		return {
			...
			c() {
				...
				t10 = text("$");
				t11 = text(t11_value);
				button0.className = "nosl dec";
				attr(input, "type", "number");
				input.min = "1";
				input.value = input_value_value = ctx.item.count;
				button1.className = "nosl inc";
				button2.className = "nosl solid-shadow";
				div.className = "overlay";
				img.className = "nosl";
				img.alt = img_alt_value = ctx.products[ctx.item.key].name;
				img.src = img_src_value = ctx.products[ctx.item.key].image;
				h2.className = "f-100";
				b.className = "bigger";
			...
			p(changed, new_ctx) {
				ctx = new_ctx;
				if ((changed.$cart) && input_value_value !== (input_value_value = ctx.item.count)) {
					input.value = input_value_value;
				}

				if ((changed.$cart) && img_alt_value !== (img_alt_value = ctx.products[ctx.item.key].name)) {
					img.alt = img_alt_value;
				}

				if ((changed.$cart) && img_src_value !== (img_src_value = ctx.products[ctx.item.key].image)) {
					img.src = img_src_value;
				}

				if ((changed.$cart) && t8_value !== (t8_value = ctx.products[ctx.item.key].name)) {
					set_data(t8, t8_value);
				}

				if ((changed.$cart) && t11_value !== (t11_value = ctx.products[ctx.item.key].price * ctx.item.count)) {
	...
	function create_fragment(ctx) {
		var h1, t1, div, ul, each_blocks = [], each_1_lookup = new Map(), t2, aside, h2, t4, p, t6, form, label0, t9, label1, t12, label2, t15, button, t16, button_disabled_value, dispose;

		var each_value = ctx.$cart.items;
		...
		return {
			c() {
				...
				label0.innerHTML = `<span>Field</span>
			        <input required type="text">`;
				t9 = space();
				label1 = element("label");
				label1.innerHTML = `<span>Field</span>
			        <input required type="email">`;
				t12 = space();
				label2 = element("label");
				label2.innerHTML = `<span>Field</span>
			        <textarea required rows="6"></textarea>`;
				t15 = space();
				button = element("button");
				t16 = text("Send request");
				h1.className = "nosl";
				ul.className = "reset";
				h2.className = "nosl biggest";
				p.className = "nosl";
			...
			p(changed, ctx) {
				const each_value = ctx.$cart.items;
				...
				if ((changed.$cart) && button_disabled_value !== (button_disabled_value = !ctx.$cart.items.length)) {
