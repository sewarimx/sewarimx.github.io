(function () {
	...
	// (28:4) {#each $cart.items as item (item.name)}
	function create_each_block(key_1, ctx) {
		var li, div, input, input_value_value, t0, button, t2, figure, img, img_alt_value, img_src_value, t3, figcaption, h2, t4_value = ctx.item.name, t4, t5, b, t6, t7_value = ctx.item.price * ctx.item.count, t7, dispose;
		...
		return {
			...
			p(changed, new_ctx) {
				...
				if ((changed.$cart) && img_alt_value !== (img_alt_value = ctx.item.name)) {
					img.alt = img_alt_value;
				}

				if ((changed.$cart) && img_src_value !== (img_src_value = ctx.item.image)) {
					img.src = img_src_value;
				}

				if ((changed.$cart) && t4_value !== (t4_value = ctx.item.name)) {
					set_data(t4, t4_value);
				}

				if ((changed.$cart) && t7_value !== (t7_value = ctx.item.price * ctx.item.count)) {
	...
	function instance($$self, $$props, $$invalidate) {
	  ...
	  function rm(item) {
	    ...
	    $cart.items = $cart.items.filter(x => x.name !== item.name); cart.set($cart);
